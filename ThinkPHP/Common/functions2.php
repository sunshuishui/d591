...
function get_access_token(){
	...
	if(time()>($lasttime+7200)){
		...
		$resault=json_decode($res,true);
		$data['accesstoken']=$resault['access_token'];
		$data['timeout']=time();
		$back=$m->where('id=1')->save($data);
		if($back){
			return $resault['access_token'];
		}else{
			exit('error access_token1');	
		}
	}else{
		if($tokeninfor['accesstoken']){
			return $tokeninfor['accesstoken'];
		}else{
			exit('error access_token2');		
		}
...
function zdbutton(){
	...
	$data = '{
		 ...
		 {    
			  "type":"view",
			  "name":"个人中心",
			  "url":"https://open.weixin.qq.com/connect/oauth2/authorize?appid='.C('APPID').'&redirect_uri=http%3a%2f%2fm.slwlvpai.com%2fmember&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect"
		  },
		  {
			   "type":"view",
			   "name":"服务中心",
			   "url":"https://open.weixin.qq.com/connect/oauth2/authorize?appid='.C('APPID').'&redirect_uri=http%3a%2f%2fm.slwlvpai.com%2fjcon&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect"
		  },
		  {
			   "type":"view",
			   "name":"联系我们",
			   "url":"http://www.aosituoma.net/index.php?m=Index&a=mkcall&tel=4000876671"
		  }]
	 }';
...
function getopenid($code){
	...
	$res=http_request($url);
	$resault=json_decode($res,true);	
	return $resault['openid'];
}

//生成带参数的二位码
//临时二维码
//$c 是自定义参数
function get_qrcode($c){
	$url='https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.get_access_token();
	$data='{"expire_seconds": 1800,"action_name": "QR_SCENE","action_info": {"scene": {"scene_id": '.$c.'}}}';
	$res=http_request($url,$data);
	$resault=json_decode($res,true);	
	return $resault['ticket'];
}
